        ...
        function controller() {
            ...
            commentsListener = SocialWallService.getPostCommentsRef().where('postId', '==', postId).orderBy('timestamp', 'asc').onSnapshot(function(querySnapshot) {
                ...
                if (querySnapshot.docs.length) {
                    ...
                    $('.add-comment-page .main-container').html(`<div class="empty-image-outher"><div class="empty-image-inner"><img src="img/empty-states/no_comments.svg"/><p>There are no comments yet! Add some.</p></div></div>`);
                }
                ...
                if ($('.add-comment-page .single-comment-container').length > 0) {
                    querySnapshot.docChanges().forEach(function(change) {
                        if (change.type === "added") {
                            ...
                            if ($('.add-comment-page .single-comment-container').eq(-2).isInViewport()) {
                                scrollListing();
                            }
                        }
            ...
            function addNewComment(comment) {
                let user = allUsersArray.filter((u) => { if (u.id === comment.userId) { return u; } })[0];
                let commentHTML = `
            ...
            function scrollListing() {
                $('.add-comment-page').stop().animate({ scrollTop: $('#social-wall-comments-list')[0].scrollHeight }, 500);
            }
